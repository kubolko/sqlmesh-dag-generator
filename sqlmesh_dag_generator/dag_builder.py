...
class AirflowDAGBuilder:
    ...
    def _build_dag_definition(self) -> str:
        ...
        schedule = (
            f'"{self.config.airflow.schedule_interval}"'
            if self.config.airflow.schedule_interval
            else "None"
        )
        tags = repr(self.config.airflow.tags)
        description = (
            f'"{self.config.airflow.description}"'
            if self.config.airflow.description
            else f'"SQLMesh DAG: {self.config.airflow.dag_id}"'
        )
...
dag = DAG(
    ...
    def _build_tasks(self) -> str:
        ...
        for model_info in self.dag_structure.models.values():
    ...
    def _build_python_task(self, model_info: SQLMeshModelInfo, task_id: str) -> str:
        ...
        model_cron_tz = (model_info.cron_tz or "").replace('"', '\\"')
        model_interval_min = (
    ...
    def _build_kubernetes_task(self, model_info: SQLMeshModelInfo, task_id: str) -> str:
        ...
        env_block = ",\n        ".join(
            [f'V1EnvVar(name="{k}", value="{v}")' for k, v in env_vars.items()]
        )
...
try:
    ...
    for model_name, model in ctx.models.items():
        ...
        _tz = getattr(model, "cron_tz", None)
        discovered_models[model_name] = {{
            "fqn": model.fqn,
            "name": str(model.name),
            "dependencies": deps,
            "cron": getattr(model, "cron", None),
            "cron_tz": getattr(_tz, "key", None) or (str(_tz) if _tz else None),
...
{auto_schedule_code}
    ...
    def _build_dynamic_dag_definition(self) -> str:
        ...
        description = (
            f'"{self.config.airflow.description}"'
            if self.config.airflow.description
            else f'"Dynamic SQLMesh DAG: {self.config.airflow.dag_id}"'
        )
...
with DAG(
    ...
    for model_name, model_info in discovered_models.items():
        ...
        def make_callable(
            model_fqn, model_display_name, model_cron, model_interval_minutes, model_cron_tz
        ):
